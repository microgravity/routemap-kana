...
export function RouteMap({ route, stationIds, mode, zoom, celebrateStationId, onSelect }: Props) {
  const { stationById, state } = useAppState()
  const width = Math.max(980, 140 + stationIds.length * 148)
  const y = 142
  ...
  return (
    <div className="route-map-scroll" aria-label={`${route.name}の しゅうろくくかん`}>
