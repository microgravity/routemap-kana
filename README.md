...
東急電鉄の9路線・公式99駅相当と、相模鉄道（相鉄）の3路線・27駅を全区間収録しています。同じ実在駅は会社や路線をまたいでも一つのStation IDへ統合しているため、組み込みStationレコードは123件です。路線図は駅順を表す独自の模式図で、地理的な距離を表す地図ではありません。
