...
東急電鉄の9路線・公式99駅相当、相模鉄道（相鉄）の3路線・27駅、東京メトロの9路線・180駅、都営地下鉄の4路線・106駅を全区間収録しています。同じ実在駅は会社や路線をまたいでも一つのStation IDへ統合しているため、組み込みStationレコードは334件です。路線図は駅順を表す独自の模式図で、地理的な距離を表す地図ではありません。
...
東京メトロは東急9路線と相鉄3路線を通常クリアすると解除され、最初は銀座線と半蔵門線から始まります。各路線で練習した文字位置が25・50・75・100%へ達するたびに区間スタンプを獲得し、最初は2個、その後は3個ごとに好きなメトロ路線を1本選んで開けます。都営地下鉄は東京メトロの好きな3路線を通常クリアすると解除され、浅草線から開始します。都営の区間スタンプ1・3・5個で、残る3路線を好きな順に開けます。みなとみらい線、JR線など各社の直通先は含みません。
...
- 駅名を書き終えたら、路線図を経由せず次の駅の練習へ進める連続練習
...
- 左利き・右利き、音声、動きを減らす設定
- 「おうちのひと」で、日ごとの練習駅数・文字数・書いた文字・お手本なし回数を確認
...
筆跡そのものは永続保存しません。「できた」にした文字位置、お手本なしで書けた文字位置、次回の再開位置、駅追加状態を区切りごとに保存します。「できた」を押した日時、駅、文字、練習モードは日別表示用の`practiceHistory`へ追記し、同じ駅は1日の中で一度だけ数えます。履歴は直近5,000回分を保持し、この機能を追加する前の練習には正確な日時がないため遡って生成しません。従来の保存データには、お手本なし進捗と日別履歴を空の状態で補い、そのまま読み込めます。
