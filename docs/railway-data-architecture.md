...
| `src/data/operators/toei/` | 都営地下鉄の駅、路線、全国版データセット |
...
現在は4社25路線・組み込みStation 334件で、初期画面、索引、サイトマップ生成を単純な静的データとして扱う方が保守しやすいため、会社データの遅延読み込みは導入していません。次のいずれかに達した時点で、会社単位の動的importと索引用軽量メタデータを再検討します。
