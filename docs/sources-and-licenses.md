...
駅名、駅番号、駅順、所属路線、読みは東急電鉄、相模鉄道、東京メトロ、東京都交通局の公式情報を基準にしました。公式路線図画像、企業ロゴ、公式の図上配置は使用していません。アプリの会社印、路線図、駅印、電車は独自の模式表現です。
...
東京メトロ公式の9路線・180駅を対象とし、各路線ページに掲載された駅順では185件の路線別駅所属を保持しています。同じ東京メトロ駅を路線ごとに重複登録せず144のStation IDへ統合し、さらに渋谷・中目黒・目黒は既存の東急Station IDを共有するため、新設Stationレコードは141件です。ここまでの3社では21路線、会社ごとの公式駅数の単純合計は306駅、共有駅を統合した組み込みStationレコードは264件、路線別駅所属は329件です。

丸ノ内線は中野坂上から方南町方面へ分岐します。現在の横一列の模式図では、公式路線ページの掲載順に支線3駅を含む全28駅を一つの並びで表示し、画面上に分岐の注記を出します。公式路線図画像、ロゴ、図上配置は使用していません。

### 都営地下鉄

- [各駅情報（駅名・駅番号）](https://www.kotsu.metro.tokyo.jp/subway/stations/)
- [都営地下鉄の概要（4路線・106駅）](https://www.kotsu.metro.tokyo.jp/about/service/subway.html)
- [路線図・停車駅](https://www.kotsu.metro.tokyo.jp/subway/map/)
- [浅草線](https://www.kotsu.metro.tokyo.jp/subway/asakusa/)
- [三田線](https://www.kotsu.metro.tokyo.jp/subway/mita/)
- [新宿線](https://www.kotsu.metro.tokyo.jp/subway/shinjuku/)
- [大江戸線](https://www.kotsu.metro.tokyo.jp/subway/oedo/)

東京都交通局公式の浅草線20駅、三田線27駅、新宿線21駅、大江戸線38駅、計106件の路線別駅所属を収録しています。同じ都営駅を路線ごとに重複登録せず、既存の東急・東京メトロ駅も共有するため、都営4路線は99のStation IDで構成し、新設Stationレコードは70件です。4社合計では25路線、会社ごとの公式駅数の単純合計は412駅、共有駅を統合した組み込みStationレコードは334件、路線別駅所属は435件です。

大江戸線は都庁前を中心に環状部と放射部を持ちます。現在の横一列の模式図では、公式駅番号E01〜E38の順で一つの並びとして表示し、画面上に注記を出します。公式路線図画像、ロゴ、図上配置は使用していません。
